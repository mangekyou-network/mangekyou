...
use crate::{
    error::MangekyouError, hash::HashFunction, serialize_deserialize_with_to_from_byte_array,
};
...
use mangekyou_derive::GroupOpsExtend;
...
impl RistrettoPoint {
    ...
    /// Return this point in compressed form.
    pub fn decompress(bytes: &[u8; 32]) -> Result<Self, MangekyouError> {
...
impl MultiScalarMul for RistrettoPoint {
    fn multi_scalar_mul(scalars: &[Self::ScalarType], points: &[Self]) -> MangekyouResult<Self> {
...
impl Div<RistrettoScalar> for RistrettoPoint {
    type Output = Result<Self, MangekyouError>;
...
impl TryFrom<&[u8]> for RistrettoPoint {
    ...
    fn try_from(bytes: &[u8]) -> Result<Self, MangekyouError> {
        let point = ExternalCompressedRistrettoPoint::from_slice(bytes);
        let decompressed_point = point.decompress().ok_or(MangekyouError::InvalidInput)?;
...
impl ToFromByteArray<RISTRETTO_POINT_BYTE_LENGTH> for RistrettoPoint {
    fn from_byte_array(bytes: &[u8; RISTRETTO_POINT_BYTE_LENGTH]) -> Result<Self, MangekyouError> {
...
impl Div<RistrettoScalar> for RistrettoScalar {
    type Output = Result<RistrettoScalar, MangekyouError>;

    fn div(self, rhs: RistrettoScalar) -> Result<RistrettoScalar, MangekyouError> {
        let inv = rhs.inverse()?;
        Ok(self * inv)
    }
}
...
impl Scalar for RistrettoScalar {
    ...
    fn inverse(&self) -> MangekyouResult<Self> {
