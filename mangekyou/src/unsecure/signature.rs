...
impl ToFromBytes for UnsecurePublicKey {
    fn from_bytes(bytes: &[u8]) -> Result<Self, MangekyouError> {
        let bytes_fixed: [u8; PUBLIC_KEY_LENGTH] = bytes
            .try_into()
            .map_err(|_| MangekyouError::InputLengthWrong(PUBLIC_KEY_LENGTH))?;
        Ok(Self(bytes_fixed))
    }
}
...
impl VerifyingKey for UnsecurePublicKey {
    ...
    fn verify(&self, msg: &[u8], signature: &UnsecureSignature) -> Result<(), MangekyouError> {
        let digest = sign(self.0, msg);
        if ToFromBytes::as_bytes(signature) == digest.0 {
            return Ok(());
        }
        Err(MangekyouError::InvalidSignature)
    }
...
impl ToFromBytes for UnsecureSignature {
    fn from_bytes(bytes: &[u8]) -> Result<Self, MangekyouError> {
        let bytes_fixed: [u8; SIGNATURE_LENGTH] = bytes
            .try_into()
            .map_err(|_| MangekyouError::InputLengthWrong(SIGNATURE_LENGTH))?;
        Ok(Self(bytes_fixed))
    }
}
...
impl ToFromBytes for UnsecurePrivateKey {
    fn from_bytes(bytes: &[u8]) -> Result<Self, MangekyouError> {
        let bytes: [u8; PRIVATE_KEY_LENGTH] = bytes
            .try_into()
            .map_err(|_| MangekyouError::InputLengthWrong(PRIVATE_KEY_LENGTH))?;
...
impl FromStr for UnsecureKeyPair {
    type Err = MangekyouError;

    fn from_str(s: &str) -> Result<Self, Self::Err> {
        let kp = Self::decode_base64(s).map_err(|_| MangekyouError::GeneralOpaqueError)?;
        Ok(kp)
    }
}
...
impl AggregateAuthenticator for UnsecureAggregateSignature {
    ...
    fn add_aggregate(&mut self, signatures: Self) -> Result<(), MangekyouError> {
    ...
    fn verify(
        ...
        Err(MangekyouError::GeneralOpaqueError)
    }
